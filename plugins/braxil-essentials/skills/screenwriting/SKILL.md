...
- If the end goal is a produced VIDEO: **`activate_skill` `cinematic-video-prompt-engineer` NOW** — it writes every scene, and citing it without activating it is how scenes end up written by the wrong craft (a reported bug). Do not wait to see it in the skills catalog: with any skill already active the catalog shows NAMES ONLY, so its description is not in your context. Then: this skill produces the HIGH-LEVEL story ONLY — premise, structure, sequences, the scene map and the characters. Do NOT write the scenes, the `action` or the `dialogue`, and do NOT design the planos: each scene is written ENTIRELY by `cinematic-video-prompt-engineer` (it renders more naturalistic scenes + dialogue than a page script). Do not hand-build storyboard JSON here, and do not substitute a script when the user asked for a storyboard (or vice versa; when ambiguous, ask).
  - Your **sequences** (each a group of scenes with one purpose) are the production unit that becomes a `storyboard` sequence downstream: one sequence → one storyboard `.json`, and the ordered set is the story's **Project** (the `storyboard` skill owns that mapping). Keep your sequence boundaries clean — they are where the story will be split into storyboards.
