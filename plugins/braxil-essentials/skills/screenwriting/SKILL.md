...
| [references/visual-grammar.md](references/visual-grammar.md) | The script or feedback involves the CAMERA: shot choices, POV, coverage, continuity, action sequences. Shared with the `storyboard` skill. |
...
- If the end goal is a produced VIDEO: this skill produces the story/script; the `storyboard` skill turns it into the storyboard artefact. Do not hand-build storyboard JSON here, and do not substitute a script when the user asked for a storyboard (or vice versa; when ambiguous, ask).
