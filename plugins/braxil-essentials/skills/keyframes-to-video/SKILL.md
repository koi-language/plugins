...
> **🔴 Step 0 — ACTIVATE `video-generator` BEFORE ANYTHING ELSE (MANDATORY, ABSOLUTE FIRST).** Before you make ANY decision, pick or even NAME a model, reason about what a model can do, OR ask the user a single question, **activate the `video-generator` skill and READ its `references/models.md`.** That skill's model cards are the ONLY authority on what each video model actually does and the exact parameters it accepts. You do NOT know these capabilities from memory — **NEVER state, assume, or reason about a model's behaviour (e.g. how it animates, whether it can chain cuts, how many clips something "would need") without having READ that model's card.** If a model is NOT in the cards, do NOT propose it or plan around it. This gate comes BEFORE Step 0a (asking the user) so you never present the user a plan built on invented model behaviour. (`video-generator` = how the TOOL + models work; the craft skill = how to WORD a clip — the craft skill does NOT replace this.)
> **Step 0b — PICK the model from the cards, then READ its card in full (only AFTER Step 0).** Choose the best model for THIS clip based on what the cards say — never from memory, a model name, or a fixed label. When a clip has several hard-cut shots, the model's card must mark it able to render them in one generation; go by the card, never assume. Once picked, READ that model's card and let IT define the model's capabilities, limits and parameters — then use the card BOTH to (a) PLAN how to structure the clips for that model AND (b) decide which questions to ask the user. **Do NOT restate or assume any technical spec in this skill; the card is the single source of truth (this skill must never hardcode a model's behaviour or params — that only drifts and contradicts the cards).**
> **Step 0a — ASK THE USER (MANDATORY, gating) — the questions the CHOSEN model's card implies.** Always ask aspect ratio (a VIDEO decision, NOT a storyboard property — the same storyboard → many formats): which format(s) they want (16:9 YouTube/web, 9:16 Reels/TikTok/Shorts, 1:1 or 4:5 Instagram, …) via `prompt_user`/`prompt_form`, never a `print`. Beyond that, ask ONLY what the model's card shows the model exposes — never ask about (or invent) params the card doesn't list. Do this ONLY here (at video generation), NEVER at storyboard time. Use their answer as `aspectRatio` on every clip; several formats → several passes; ask once, up front. Do not proceed to generate without this answer.
> **If the chosen model's card names a dedicated craft skill, activate it and follow THAT skill's own instructions for wording.** The craft skill routes to its own sub-skills/references — read whatever IT directs (activating the root but never reading what it points to = did NOT use it). Which models have a craft skill, and its name/how to activate it, is stated on the model's card in `video-generator/references/models.md` — this skill does not name models.
> **No craft skill named on the card →** use the compact shape below (self-sufficient). It's an enhancement, not a dependency; don't tell the user to install anything.
