...
Sanity-check EACH clip (`durationSec` or summed `shot.duration`). Any clip > 15 s (or no `metadata.clips` AND whole sheet's shots sum > 15 s) → upstream chunking bug; don't silently drop shots (*"hizo los 15 s pero solo de una parte"* bug). STOP: *"Clip `<clipIndex>` sums to `<X>` s but a rendered clip caps at 15 s. Please re-build the sheets with `keyframes`: its Chunking step splits shots into ≤15 s clips I'll then render and concatenate."* Never auto-compress/truncate.
...
2. **Per panel, decide the LAST model it passes through:**
   ...
   - **Panel has NO human face** (or a stylised anime / 2D / clay / cartoon face — those don't trip the filter):
...
- Attach a raw photoreal human face to Seedance (it rejects it → failed render) — route every face panel through an EXACT Seedream copy first (pre-step); the Seedream image is what clears the filter.
