...
> 1. **This clip's OWN sheet, WHOLE and UNCROPPED, converted to a DEPTH MAP** (one image) — it carries the SCENE STRUCTURE of every panel at once: composition, blocking, camera distance, where things sit in space.
> 2. **The CHARACTER SHEET(s) — a model-sheet TURNAROUND per character appearing in the clip (2×4: 4 full-body views + 4 head close-ups)**. 🚨 **These, and ONLY these, are MANDATORY through Seedream** — they are the only faces you attach, and a raw photoreal face gets the whole render rejected.
...
> - **🔗 CONTINUITY between shots.** A depth map hands Seedance the spatial layout **without imposing a concrete frame anywhere**. Nothing in the clip is pinned to a literal still, so the model is free to FLOW from shot to shot — motion, lighting and world-state carry across the cuts instead of the render snapping to each panel and restarting. A photoreal panel does the opposite: it forces an exact frame at each point and fights that flow, which is where jumpy, disconnected cuts come from. Structure guides, a frame dictates — we want guidance.
> - **🛡️ The face filter.** The depth sheet contains **no face at all**, so the likeness filter cannot fire on it. Identity comes from the character turnarounds instead — those are Seedream images, which is what clears the filter.

Per clip:

1. **Depth map of the SHEET** — `generate_image` in EDIT mode with **this clip's whole sheet** as the reference image. **🔥 NO CROPPING, NO WARPING, NO RE-FRAMING: the sheet must come back EXACTLY as it is** — same grid, same panels in the same cells, same composition inside each panel. Pass the sheet's own `width`/`height`/`aspectRatio` so it returns at the same shape and size. **Use this prompt (verbatim):**
   > `Convert this image into a grayscale DEPTH MAP. Near surfaces bright white, far surfaces dark black, smooth continuous gradient with distance from camera. Preserve the layout, the grid of panels, every panel's composition, framing, subject positions, poses and silhouettes EXACTLY 1:1 — do not move, add, remove, re-frame, merge or restyle anything. Output ONLY depth: no colour, no texture, no facial features, no detail — just the depth gradient.`

   (This is an APPROXIMATED depth map produced by an image model, not a metric depth pass — accepted and intended here; it only has to convey spatial layout.)
...
The attach list is then: **the depth-map SHEET** + **the Seedream'd TURNAROUND of each character in this clip** + **the object/location refs the clip needs** (as-is, no Seedream). A photoreal panel sheet is NEVER attached, and panels are NEVER extracted.
...
> 1. **This clip's DEPTH-MAP SHEET** (from the pre-step) — ONE image, whole and uncropped, alias e.g. `depth_sheet`. It is STRUCTURE only: composition, blocking, spatial layout of every panel. **Never attach the photoreal sheet, never extract panels**, and **never attach another clip's sheet**. The prompt names the panel range ("panels A to B") and the model maps shots→panels itself.
> 2. **The Seedream'd TURNAROUND SHEET of each character appearing in THIS clip** — one per character, aliased by character label (e.g. `HERO_A`). They are IDENTITY: the depth sheet deliberately carries no look, so faces, hair, wardrobe and proportions all come from here. Attach only the characters actually in this clip.
...
> 4. **The immediately-previous clip as a video (K ≥ 2 ONLY)**: `[{ alias: "prev_clip", path: <K-1's savedTo> }]` in `referenceVideos`. IMMEDIATE predecessor only — never earlier clips. ⚠️ That video DOES carry faces, so it can still trip the face filter; if a chained render is rejected for likeness (everything else being Seedream'd/depth), swap it for a **faceless continuity frame** (`extract_frame` a hands/object/scenery frame from the prior clip).
>
...
>
> **In the prompt, say what each reference IS** so the model uses them correctly — e.g. *"`depth_sheet` is a DEPTH MAP of this clip's panels (panels 4–7): use it for composition, camera distance and where subjects sit in space, not for look. `HERO_A` is her character turnaround sheet: take her face, hair and wardrobe from it. `SET_suite` is the location plate: the room must match it."*
...
**Faces need NO strip line.** The sheet goes in as a DEPTH MAP (no face at all) and the character turnarounds are Seedream copies (clean ordinary faces) — there is no mesh, grid or overlay to negate. Do not add any "bare skin / strip the mesh" line; that method is gone.
