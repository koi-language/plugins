...
- **SUPER MANDATORY — DO launder EVERY panel with a photoreal human face OR ANY part of one through Seedream before attaching, ALL of them up front.** Seedance rejects a raw photoreal face as a reference (the whole render fails with *"image_urls: may contain likenesses of real people"*), but ACCEPTS a Seedream-generated image. "A face-part" is BROAD: frontal, profile, from behind with any feature showing, partial/cropped, small/background, blurred/shadowed, reflected, eyes open or closed — when in doubt, launder. Pre-step (main skill): `extract_panel` each face panel → `generate_image` EDIT with `bytedance/seedream/v5/pro/edit` for an EXACT 1:1 copy → attach that Seedream copy. The face panel's LAST model MUST be Seedream (at 4K use Seedream's max size, do NOT upscale a face panel). No mesh, no strip line — the Seedream copy is already a clean ordinary face. **Also: the chained previous-clip reference VIDEO carries the face too — if a fully-laundered render still rejects for likeness, the video is the trigger; replace it with a faceless `extract_frame` continuity anchor.**
