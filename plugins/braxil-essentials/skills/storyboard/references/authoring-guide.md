...
- **`continuity`** (root LOCK, string[]): absolute story-wide INVARIANTS + NEGATIVE constraints, injected VERBATIM into every image/video prompt. `synopsis`=WHY; `continuity`=WHAT MUST NOT CHANGE. Spell out negatives ("do NOT lower the button", "cans stay INTACT") to stop renderer making unreachable reachable, resizing, breaking objects. Omit only for a bare shot list.
  - **🔴 STAGING / AXIS LOCK for a fixed-location multi-character scene (dinner table, car, sofa…): MANDATORY as a LOCK line.** The video model reinvents the geometry on every hard cut and flips who sits where (silent 180-degree axis cross — the reported "she was beside the father, two shots later across from him" bug). When cinematic's scene fixes a camera side + screen positions, TRANSCRIBE that into the LOCK verbatim so it rides into every clip prompt, e.g.: *"La cámara permanece SIEMPRE en el mismo lado de la mesa; a cámara: MADRE a la IZQUIERDA, HIJA a la DERECHA, PADRE al fondo de frente. Nadie cambia de lado de pantalla en ningún plano; no se cruza el eje de 180°."* If cinematic didn't state it, DERIVE it from shot 1's blocking and add it. (Per-shot, ALSO keep each visible character's screen side in that shot's `composition` — the LOCK is the global anchor, `composition` restates it per cut.)
