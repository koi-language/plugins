...
- **`characters`** (roster): each recurring char: LABEL + silhouette desc (clothing, proportions, accessories, traits), NOT faces. Label `SHORT_UPPERCASE`+A/B/C (`HERO_A`); reuse EXACTLY in every `action`/`dialogue`, never "she". Compositor (render-time LLM) inflates label→inline English desc; define once, don't restate per shot.
...
Scope cascade (specific wins, broader still applies): root → EVERY shot; scene → every shot in scene; shot → that shot. At render, visual-panels skills collect in-scope refs per shot, pass as `referenceImages` (`@handles`→paths) to lock identity. Omit when empty; never `"references": []`.
