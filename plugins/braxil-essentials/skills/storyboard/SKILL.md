...
10. **Storyboard → sheets consistency is ABSOLUTE: PROPAGATE BY DEFAULT.** If rendered keyframe sheets exist (creations with `metadata.sourceStoryboard` → this), any modification makes affected cuadros STALE and you MUST update the visual side IN THE SAME TURN, unasked: content-level shot changes → `keyframes` fixing flow on affected cuadros; shots added/removed/reordered or scene-level → re-render affected sheet(s) with a NEW `sheetSetId`. Skip only if the user EXPLICITLY defers ("luego lo regeneramos"), then state which cuadros/sheets are out of date. Reverse direction (in `keyframes`): story-affecting changes against sheets land HERE first, then propagate back.
