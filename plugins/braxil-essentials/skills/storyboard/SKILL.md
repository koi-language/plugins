...
- Three ways to write it, pick by the SHAPE of the edit — a storyboard is a JSON FILE and may be edited as text like any other:
  - **`update_storyboard`** — the default for routine edits: content fields AND shot add/remove/reorder, addressed by id, with renumbering handled for you; only the affected panels re-render.
  - **`edit_file` / `write_file`** — legitimate, and the BEST tool when the change is textual and spread out: renaming a character across every shot, a find-and-replace of a recurring phrase, or any surgical change easier to express as text than as a field patch. A post-write guard re-parses and re-validates the document and RESTORES the previous bytes if the edit broke it, so a bad edit can never leave the visor with an unrenderable file. Read the file first, as with any edit.
  - **`save_storyboard`** — creating a storyboard, and scene-level restructuring (adding/removing/reordering whole scenes) (derives `version`/`createdAt`/`updatedAt`/path, validates, escapes JSON).
...
1. **Invoke the two craft skills FIRST — story ARCHITECTURE, then SCENE realization.** A storyboard IS the film pre-visualised, so BEFORE authoring content:
   - **🔴 ACTIVATE THEM, do not just cite them.** `activate_skill` `screenwriting` AND `cinematic-video-prompt-engineer` NOW, before writing a single field. Naming a skill in your reasoning is not using it: the craft lives in its body, and an unactivated skill contributes nothing. **Do NOT wait to see them in the skills catalog** — once ANY skill is active the catalog collapses to NAMES ONLY, so their descriptions (and their MANDATORY wording) are no longer in your context. Activate from THIS line. The ONLY case where cinematic need not be active here is when you delegate every scene to the isolated `scene-writer` agent, which carries it in its own frontmatter.
   - **(a) `screenwriting` — the HIGH-LEVEL story** (Skill tool `braxil-essentials:screenwriting`): the premise/theme, the STRUCTURE (whatever dramatic shape ITS craft judges right for the material — this skill never prescribes a shape or a template), the SEQUENCES and the SCENE MAP (which scenes exist, their order, each scene's dramatic FUNCTION / goal / turning point, its location and which characters are in it), the CHARACTERS (bible, arcs, voice) and the story-wide continuity LOCK. It does NOT write the shot-level content.
   - **(b) `cinematic-video-prompt-engineer` — get its NATIVE scene output, THEN transliterate it. TWO steps, never merged:**
     ...
     - **(i-bis) 🔴 SCENES ARE WRITTEN IN ORDER, IN ONE CONTINUED CONVERSATION — NEVER IN PARALLEL.** Scene-to-scene continuity lives in the WRITTEN scene (the actual dialogue lines, the staging the writer chose, the physical/emotional state it ends on) — none of which exists yet while the previous scene is still being written, so fanning scenes out to parallel scene-writer agents LOSES the continuity between scenes (reported bug). Choreography, exactly like a user driving the cinematic skill by hand:
       - **Scene 1:** spawn ONE `scene-writer` agent with scene 1's natural prose prompt.
       - **Scene N≥2:** CONTINUE THE SAME agent (SendMessage to the agent id/name the Agent tool returned) with a natural continuation turn — "continúa: la siguiente escena — …" plus that scene's prose (what happens, who, where, the turn). The agent has its own previous scenes in context; its skill's Continuation Workflow chooses the bridge and keeps state, emotion and voice progressing. Do NOT re-send the story premise or re-describe unchanged characters/sets each turn — it already has them.
       - **Fallback — fresh agent with the prior scene verbatim:** only when continuing is impossible (no SendMessage in this backend, the agent is gone, or the user asked to REDO a scene from scratch): spawn a fresh `scene-writer` whose prompt INCLUDES the previous scene's output VERBATIM ("la escena anterior quedó así: «…»") followed by the new scene's prose phrased as a continuation. Never hand scene N≥2 to any writer without the prior WRITTEN scene in its prompt.
       - Transliterating scene N (step ii) may overlap with the writing of scene N+1, but two scenes must NEVER be being written at the same time.
     - **(ii) SHOW the user what the agent returned, THEN transliterate.** First **`print` the scene the agent wrote** — its shot breakdown (per shot: framing, action, and the dialogue lines) — so the user sees the actual cinematic output, NOT a one-line summary like "the agent returned the scene". Then **TRANSLITERATE that output into the JSON**: each `镜头`/shot cinematic wrote → one storyboard shot; map its parts to the fields: shot-size+angle → `shot`, camera-move+lens/speed/focus → `camera`, composition → `composition`, light → `lighting`, performance/action → `action`, the spoken line → `dialogue`, sound → `sfx`/`music`, timing → `duration`, continuous-take → `noCutBefore`. **🔴 `composition` and `lighting` must NOT end up empty when the scene states them — EXTRACT, don't just match labels.** Cinematic often carries the composition INSIDE the prose (the staging lock, screen-LEFT/RIGHT positions, foreground/out-of-focus elements, depth, eyeline) and states the light ONCE at scene level: pull each shot's in-frame layout out of its prose into `composition`, and write into each shot's `lighting` how the scene's light design reads in that shot (direction/quality/effect — a compact line; identical wording across shots is fine when the light doesn't change). This is a FAITHFUL transliteration: do NOT re-decide, drop, add, reorder, merge, pad or "improve" anything — you EXPRESS cinematic's scene verbatim in intent, in the user's language.
...
13. **Every scene MUST be associated with a location card — MANDATORY (a scene = its characters + its location(s)).** Each scene needs a `scene.locationIds` ARRAY linking one or more saved `~/.koi/locations/<id>.json` cards (with generated establishing plates) — a scene can play across SEVERAL sets, so it's a list. The card + its plate are what lock each SET's look/palette/lighting across every clip of that scene (a text `location` string does not). Also build the storyboard `locations` roster (`[{ name, description?, locationId? }]`). When you author or finish a storyboard whose scenes have NO linked location, you MUST resolve it — ASK the user via `prompt_user`/`prompt_form` (NEVER a `print`): *"do you want to ASSOCIATE existing location(s), or shall I INVENT and create new ones?"*
   ...
   - **Invent** → for EACH distinct location, via the `locations` skill: (1) `save_location` with a `name` + `description` (+ `locationType`/`timeOfDay`/`palette`/`lighting` when known); (2) `generate_location_plate` with `view:"front"` to build and attach its ESTABLISHING PLATE (pass `locationId`) — the FRONT view only; rear/left/right are optional angles, generated later only if a scene needs one; (3) add the returned id to `locationIds` of every scene set there + the roster. Do all three — a location invented with no card or no plate is incomplete.
...
|---|---|
| `.../story-structure.md` | Inventing/restructuring: screenwriting's structural craft (want+obstacle, turning points, climax) applied at whatever shape IT judges right. The scene progression IS the structure; check vs its checklist. |
...
1. **Invoke `screenwriting` first for the HIGH-LEVEL story** (rule 1a): develop the premise, the dramatic STRUCTURE and the SCENE MAP (the ordered list of scenes with each one's function/goal/location/characters) and the characters. The SHAPE of the structure is entirely screenwriting's call, per its own craft ("structure is a diagnostic, not a paint-by-numbers kit") — this flow never prescribes a shape, a template or an act count; a gag, a loop, a montage or a mood/product piece may carry no dramatic arc at all. **Then DELEGATE the scenes to the isolated `scene-writer` agent, IN ORDER through ONE continued conversation — scene 1 spawns the agent, every next scene is a "continúa" turn to that SAME agent (rule 1b(i-bis); NEVER parallel agents — that loses scene-to-scene continuity):** it writes each scene natively (`action`, naturalistic `dialogue`, the planos, `noCutBefore`, `sfx`/`music`, durations) with its own previous scenes in context; you TRANSLITERATE each output into the shots as it arrives (fallbacks per rule 1b(i)/(i-bis): fresh agent with the prior scene verbatim, or cinematic inline).
...
0. **🔴 `activate_skill` BOTH craft skills BEFORE touching anything** (rule 1) — `screenwriting` AND `cinematic-video-prompt-engineer`. ACTIVATE them; citing a skill in your reasoning is not using it.
   - **Both is the DEFAULT, not a judgement call.** The old wording split them by edit type (`screenwriting` only for "high-level story" edits) and it failed in practice: a review like *"revisa la historia, mira si la SECUENCIA es correcta, los planos y los tiempos"* is BOTH — whether the sequence works is story architecture (`screenwriting`), whether the planos and timings read well is scene craft (`cinematic`). Asked to judge or fix a storyboard, you cannot know which of the two you need until you have already looked with both.
   - **Any open-ended request activates both**: "revísalo", "mira si está bien", "mejóralo", "que quede fluido/cinematográfico", "corrige lo que consideres". So does any change to scene ORDER, scene COUNT, pacing across scenes, or the arc.
   - Skip a craft skill ONLY for a narrow, explicitly-scoped mechanical edit ("cambia esta palabra del diálogo del plano 4", renaming, fixing a path). When in doubt, activate both — the cost is one call, the cost of not doing it is a review done without the craft.
...
3. **SHOT-level STRUCTURE (add / remove / reorder shots) → ALSO `update_storyboard`.** It now carries `addShots: [{ shot: {…}, afterShotId? | sceneId?, index? }]`, `removeShots: ["sh4"]` and `moveShots: [{ id, afterShotId? | sceneId?, index? }]`, and it **renumbers the whole document for you** (including the `noCutBefore` shared-number rule). Never re-send the document to insert or delete a shot. `save_storyboard` is now only for: CREATING a storyboard, SCENE-level restructuring (adding/removing/reordering whole scenes), and id changes.
   - 🔴 **Why this matters, in real numbers:** a full document is 26k–37k characters the model must WRITE OUT token by token; the equivalent patch is 0.2k–4k. That difference IS the "editing a script takes forever" the user feels. A content-only full save is now REJECTED by the tool with the list of fields that actually changed — build the patch from that list.
4. `number` must stay sequential across ALL scenes (the shared number for `noCutBefore` continuations). `update_storyboard`'s structural ops do this automatically; only a `save_storyboard` full write makes it your job.
...
- Write storyboard JSONs outside `~/.koi/storyboards/` (the visor only watches that folder). Inside it, any of the three write paths above is fine.
