...
- **Character turnaround** — build ONE per character with **`generate_character_sheet`** (see the `characters` skill) from that character's reference image, or from the storyboard's `characters` description if none: a model-sheet turnaround (full-body + head views on a neutral studio backdrop) that locks identity. **Prepare it per the chosen video model's card** (some models need it laundered/processed before it's an acceptable reference). Build once, reuse across every clip.
- **Set/location plate** — if the location recurs and no plate exists, generate one establishing plate and persist it (`scene.references` and/or a `locations` Library asset), then attach it wherever that location is in frame.
...
- **⚠️ Framing/composition lives ONLY in this brief / the cinematic prompt — ALWAYS** (the composed sheet is never attached to the model). With no panel image driving the model, be explicit about each shot's shot-size, angle and camera move so cinematic (and the model) have the composition from words alone.
