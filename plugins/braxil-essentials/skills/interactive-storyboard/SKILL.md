...
7. **Duration: storyboard is the single timing authority.** Summed `duration`s ARE the runtime. A user's upfront duration only sizes the FIRST draft (60s = 8-12 shots of 4-8s). After that the current total wins: re-read and re-sum before reporting/rendering; never silently retime to re-hit an old target. Only an explicit current hard-target ("que dure exactamente 30s") authorises retiming, and say what you retimed.
...
9. **Camera-movement shots: split into a glued take.** A moving shot >1-2s → 2-4 panels describing the move beat by beat, `"noCutBefore": true` on every panel after the first (render as ONE clip). Never on a scene's first shot.
10. **Storyboard → sheets consistency is ABSOLUTE: PROPAGATE BY DEFAULT.** If rendered visual panel sheets exist (creations with `metadata.sourceStoryboard` → this), any modification makes affected cuadros STALE and you MUST update the visual side IN THE SAME TURN, unasked: content-level shot changes → `visual-panels` fixing flow on affected cuadros; shots added/removed/reordered or scene-level → re-render affected sheet(s) with a NEW `sheetSetId`. Skip only if the user EXPLICITLY defers ("luego lo regeneramos"), then state which cuadros/sheets are out of date. Reverse direction (in `visual-panels`): story-affecting changes against sheets land HERE first, then propagate back.
...
Preferred `shot` (combine with angle, e.g. "CU low angle") and `movement`. Deeper craft: `../screenwriting/references/visual-grammar.md`.

| Shot | Framing | Use for |
|---|---|---|
| ECU | Eyes / detail | Intense emotion, reveals |
| CU | Face fills frame | Emotion, reaction, dialogue |
| MCU | Head & shoulders | Conversations |
...
| POV / First-Person | Through subject's eyes (hands visible, subject never shown); any style |
...
Continuity craft while choosing shots: keep all cameras one side of the action axis (180-degree rule; cross only via a neutral shot or visible move), match on action across cuts, eyeline matches (look, then what they see), consistent screen direction. Full diagrams: `../screenwriting/references/visual-grammar.md`.
