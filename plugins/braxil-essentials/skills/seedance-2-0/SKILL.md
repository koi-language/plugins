...
> ## 🧭 HOW TO RESOLVE `[skill:X]` AND `[ref:X]` IN THIS ENVIRONMENT — READ THIS FIRST
>
> This skill routes to its sub-skills and references with `[skill:X]` / `[ref:X]` markers. There is **no auto-resolver here** — YOU resolve them, by READING the file, relative to THIS skill's own directory (the folder that holds this `SKILL.md`):
> - **`[skill:X]`** → open **`skills/X/SKILL.md`** with the `Read` tool, then follow it. (These are bundled resource files, NOT separately-registered skills — do NOT try to call a `Skill`/`activate_skill` tool for them; they will not be found. Just Read the file.)
> - **`[ref:X]`** → open **`references/X.md`** with the `Read` tool, then use it.
>
> So when the routing below says e.g. *"route to `[skill:seedance-prompt-short]`"* or *"load `[ref:directing-engine]`"*, that means: `Read` `skills/seedance-prompt-short/SKILL.md` / `references/directing-engine.md` and apply it. A marker you never open is a route you never took — actually open the ones the current task invokes. Do not treat the markers as decoration.
