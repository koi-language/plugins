...
2. **Upscale each of this clip's panels** (the meshed crop for face panels, the plain `extract_panel` crop for the rest) with `upscale_image` to the **maximum resolution the video model accepts** (Seedance → 4k). Use the `upscaleFactor` needed to get each panel crisp at that size. Set `metadata: { "visible": false }` on these intermediate upscaled crops too (throwaway, keep them out of the creations drawer). **MUST: run the upscales IN PARALLEL** — the panels are independent, so fire all the `upscale_image` calls at once and await them together, never one-at-a-time (this is prep, unrelated to the SEQUENTIAL video-render rule, which only governs `generate_video` clips because each continues from the last).
