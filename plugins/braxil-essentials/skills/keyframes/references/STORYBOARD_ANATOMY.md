...
>
> **ONE allowed expansion (NOT invention): a shot with notable CAMERA MOVEMENT → 2–3 keyframe panels of that SAME shot** (start/middle/end) so the video generator sees continuous motion. Adds PANELS, never SHOTS/events. Still forbidden: re-decomposing the story, adding beats/reactions, padding to a per-type range, dropping shots to fit a grid. So **panel count = Σ per-shot panels (1 static, 2–3 camera-movement)**, every shot present, in order.
>
> **Camera-movement shots → 2–3 keyframe panels.** When `movement` describes the camera (or fast subject) MOVING: push-in/dolly, pan, tilt, crane, orbit, tracking/follow, whip, and it reads as motion (≈≥3 s, or any clearly non-"static/locked-off" `movement`): 2 panels (start+end) or 3 (start+mid+end) for large/long moves (≈≥5 s). Each keyframe panel: shares the SAME `shotId`, SAME action/state/setting, only camera framing progresses; caption keeps shot type + a sub-range of the timecode (0:08–0:12 dolly → A `0:08-0:10`, B `0:10-0:12`) + a short move-stage note. Gives `generate_video` start/mid/end keyframes. Static/locked-off never expands. Default-15 / per-type ranges apply ONLY with NO JSON (prose brief from scratch).
>
...
At least `ceil(totalSeconds/15)` clips. **Cap each clip at ≤12 PANELS too** (count camera-movement expansions): open a new clip the moment the next shot would exceed **EITHER 15 s OR 12 panels**. (Below, every "PART" = one CLIP; sheet-packing is Step B.)
...
1. **Panels per clip** = Σ its shots' panels (1 static + 2–3 camera-movement).
...
3. **Oversized clip** (own panels >12): trim camera-movement expansion (drop the middle keyframe on least-important moving shots) until it fits.
...
```
`clipIndex` = **global** sequential across the whole storyboard (timeline order); `panels` = this clip's 1-based panel numbers ON THIS SHEET (a camera-movement shot contributes its 2–3 consecutive panels); `durationSec` = integer duration; `shotIds` = its JSON ids. Also stamp `metadata.sourceStoryboard` (JSON path), `storyboardPart: K`, `storyboardParts: K_total`.
